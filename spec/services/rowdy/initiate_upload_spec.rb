...
module Rowdy
  RSpec.describe InitiateUpload do
    ...
    describe ".call" do
      ...
      context "with a custom chunk_size" do
        it "uses the provided chunk_size to calculate total_chunks" do
          upload = described_class.call(filename: "data.xlsx", file_size: 1000, chunk_size: 300)
          expect(upload.total_chunks).to eq(4)
        end
      end

      context "with a schema_name" do
        it "stores the schema_name on the upload" do
